...
RunFEBA = function(args = commandArgs(trailingOnly=TRUE)) {
	...
	FEBA_R = paste(FEBAdir,"/lib/FEBA.R",sep="");
        genesUsedfile = paste(dir,"/genesUsed",sep="");
	...
	if (file.access(FEBA_R, mode=4) != 0) stop("Cannot find ",FEBA_R);
        if (file.access(genesUsedfile, mode=4) != 0) stop("Cannot find ",genesUsedfile);
	source(FEBA_R);
	...
	exps = read.delim(expsfile,as.is=T);
        genesUsed = read.delim(genesUsedfile,quote="",as.is=T);
	...
	options(width=100);
	fit = FEBA_Fit(exps, all, genes, genesUsed$locusId);
